...
public class Test{
	...
	static List<String> rNameList = new ArrayList<String>();
	static List<String> rLevelList = new ArrayList<String>();
	...
	public void print(String[] arr3) {	//알고리즘에 의해 정리된배열에 저장해놓은 기본정보 중에서 입력한 재료 레시피 코드랑 겹치는 기본정보 출력
		for(int i = 0 ; i<rcpList.size(); i++) {
			...
			rNameList.add(ri.rName);
			rLevelList.add(ri.rLevel);
			for(int k = 0 ; k<rcpList.size(); k++) {
				if(ri.rCode.equals(arr3[k])) {
					System.out.println("레시피 이름 : " + rNameList.get(33));
				}	//if문
			}
		}
	}
